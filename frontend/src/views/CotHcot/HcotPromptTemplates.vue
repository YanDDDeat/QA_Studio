<template>
  <div class="page-container">
    <h2 class="page-title">多COT提示词模板管理</h2>
...
import {
  ...
  getHcotPromptTemplate,
  duplicateHcotPromptTemplate,
  renameHcotPromptTemplate,
  setDefaultHcotPromptTemplate,
  deleteHcotPromptTemplate,
  getHcotPromptItem,
  updateHcotPromptItem,
  restoreDefaultHcotPromptItem,
} from '../../api'
...
async function fetchTemplateDetail() {
  ...
  try {
    const res = await getHcotPromptTemplate(selectedTemplateId.value)
...
const selectedPromptBreadcrumb = computed(() => {
  ...
  // H-CoT doesn't have cot_type_name, just show the label directly
  return selectedPrompt.value.label
})

// H-CoT variable hint mapping
const hcotVariableHintMap = {
...
const variableHint = computed(() => {
  ...
  // Prefer variable_hint from tree data if available (backend provides it)
  if (selectedPrompt.value.variable_hint) {
    return selectedPrompt.value.variable_hint
  }
  // Fallback to computed mapping
  return hcotVariableHintMap[key] || ''
})
...
async function fetchPromptContent() {
  ...
  try {
    const res = await getHcotPromptItem(selectedTemplateId.value, selectedPrompt.value.prompt_key)
...
async function handleSavePrompt() {
  ...
  try {
    const res = await updateHcotPromptItem(
...
async function handleRestoreDefault() {
  ...
  try {
    const res = await restoreDefaultHcotPromptItem(
      selectedTemplateId.value,
      selectedPrompt.value.prompt_key
    )
    promptData.value = res
...
async function handleDuplicateConfirm() {
  ...
  try {
    await duplicateHcotPromptTemplate(selectedTemplateId.value, { name: duplicateForm.value.name })
...
async function handleSetDefault() {
  ...
  try {
    await setDefaultHcotPromptTemplate(selectedTemplateId.value)
...
async function handleRenameConfirm() {
  ...
  try {
    await renameHcotPromptTemplate(selectedTemplateId.value, { name: renameForm.value.name })
...
async function handleDelete() {
  ...
  try {
    await deleteHcotPromptTemplate(selectedTemplateId.value)
