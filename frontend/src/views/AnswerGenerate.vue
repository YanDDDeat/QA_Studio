    ...
    <el-card class="form-card">
      ...
      <div class="config-layout">
        <div class="config-form">
          <el-form :model="form" label-width="100px" :disabled="taskRunning">
            ...
            <el-form-item label="选择Prompt">
              <el-select v-model="form.prompt_id" placeholder="请选择答案生成阶段的Prompt" style="width: 100%" filterable>
        ...
        <div class="config-preview">
          <PromptPreview stage="answer_generate" :version="drawerVersion" :prompt-name="drawerPromptName" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event"
    ...
    <el-card v-if="taskInfo" class="progress-card">
      ...
      <div class="progress-area">
        ...
        <div class="progress-text">
          ...
          已生成 {{ taskInfo.generated_count }} 个答案
        </div>
      </div>
    </el-card>
...
const referenceFields = ref(['input', 'task_type', 'originContent'])
...
watch(() => form.value.file_id, async (fileId) => {
  if (!fileId) {
    fileFields.value = []
    referenceFields.value = ['input', 'task_type', 'originContent']
    return
  }
  try {
    const res = await getFileFields(fileId)
    fileFields.value = res.fields || []
    referenceFields.value = ['input', 'task_type', 'originContent'].filter(f => fileFields.value.includes(f))
  } catch {
    fileFields.value = []
  }
}, { immediate: true })
...
const {
  ...
  saveLoading,
  saveAsNewVersion,
} = usePromptDrawer('answer_generate', promptOptions, form, selectedLLMConfigId)
...
async function fetchFileOptions(showAll) {
  try {
    const res = await getAnswerGenerateSourceFiles({ show_all: showAll })
...
async function fetchPromptOptions() {
  try {
    const res = await getPromptConfigs({ stage: 'answer_generate' })
...
async function handleStart() {
  ...
  try {
    ...
    const res = await startAnswerGenerate(payload)
...
async function pollStatus() {
  if (!taskId.value) return
  try {
    const res = await getStageStatus('answer-generate', taskId.value)
    taskInfo.value = res
...
async function restoreTaskState() {
  try {
    const tasks = await getTaskList({ stage: 'answer_generate' })
