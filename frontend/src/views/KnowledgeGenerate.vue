    ...
    <el-card class="form-card">
      ...
      <div class="config-layout">
        ...
        <div class="config-preview">
          <PromptPreview stage="knowledge_generate" :version="drawerVersion" :prompt-name="drawerPromptName" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event"
    ...
    <el-card v-if="taskInfo" class="progress-card">
      ...
      <div class="progress-area">
        ...
        <div class="progress-text">
          ...
          已生成 {{ taskInfo.generated_count }} 个知识体系
        </div>
      </div>
    </el-card>
...
import {
  ...
  getTaskLogs,
  getTaskList,
  getKnowledgeGenerateSourceFiles,
  getPromptConfigs,
  getLLMConfigs,
} from '../api'
...
watch(() => form.value.file_id, (newFileId) => {
  ...
  if (file && !form.value.output_filename) {
    form.value.output_filename = buildDefaultOutputFilename(file.filename, 'knowledge_generate', username.value)
  }
})
...
async function handleConfigConfirm(data) {
  ...
  try {
    if (configAction.value === 'retry') {
      if (!taskId.value) return
      const res = await retryStage('knowledge-generate', taskId.value, payload)
      taskId.value = res.task_id
      taskRunning.value = true
      await pollStatus()
      startPolling()
