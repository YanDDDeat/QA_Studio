    ...
    <el-card class="form-card">
      ...
      <div class="config-layout">
        <div class="config-form">
          <el-form :model="form" label-width="100px" :disabled="taskRunning">
            <el-form-item label="选择文件">
              <FileSelector v-model="form.file_id" :fetch-fn="fetchFileOptions" expected-stage="answer_generate" :disabled="taskRunning" />
            </el-form-item>

            <el-form-item label="选择Prompt">
              <el-select v-model="form.prompt_id" placeholder="请选择答案校验阶段的Prompt" style="width: 100%" filterable>
        ...
        <div class="config-preview">
          <PromptPreview :version="drawerVersion" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event" :reference-fields="drawerReferenceFields" @update:referenceFields="drawerReferenceFields = $event" @save="saveAsNewVersion" />
        </div>
      </div>
    </el-card>
...
watch(() => form.value.file_id, (newFileId) => {
  ...
  if (file && !form.value.output_filename) {
    form.value.output_filename = buildDefaultOutputFilename(file.filename, 'answer_validate', username.value)
  }
})
...
const {
  ...
  saveLoading,
  saveAsNewVersion,
} = usePromptDrawer('answer_validate', promptOptions, form, selectedLLMConfigId)
...
async function fetchFileOptions(showAll) {
  try {
    const res = await getAnswerValidateSourceFiles({ show_all: showAll })
...
async function fetchPromptOptions() {
  try {
    const res = await getPromptConfigs({ stage: 'answer_validate' })
...
async function handleRetry() {
  ...
  startLoading.value = true
  try {
    const res = await retryStage('answer-validate', taskId.value)
    taskId.value = res.task_id
    taskRunning.value = true
...
async function pollStatus() {
  if (!taskId.value) return
  try {
    const res = await getStageStatus('answer-validate', taskId.value)
    taskInfo.value = res
...
async function restoreTaskState() {
  try {
    const tasks = await getTaskList({ stage: 'answer_validate' })
