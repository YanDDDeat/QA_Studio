    ...
    <el-card class="form-card">
      ...
      <div class="config-layout">
        <div class="config-form">
          <el-form :model="form" label-width="100px" :disabled="taskRunning">
            ...
            <el-form-item label="选择Prompt">
              <el-select v-model="form.prompt_id" placeholder="请选择质检阶段的Prompt" style="width: 100%" filterable>
        ...
        <div class="config-preview">
          <PromptPreview stage="quality_check" :version="drawerVersion" :prompt-name="drawerPromptName" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event"
    ...
    <el-card v-if="taskInfo" class="progress-card">
      ...
      <div class="progress-area">
        ...
        <div class="progress-text">
          ...
          <template v-if="passRateLabel">
            ，通过率 <span class="pass-rate">{{ passRateLabel }}</span>
          </template>
        </div>
      </div>
    </el-card>
    ...
    <el-card v-if="taskInfo" class="log-card">
      ...
      <div class="log-area" v-loading="logLoading">
        ...
        >
          ...
          <span :class="log.log_content.includes('质检通过') ? 'log-pass' : log.log_content.includes('质检失败') ? 'log-fail' : 'log-content'">
            {{ log.log_content }}
          </span>
        </div>
      </div>
...
watch(() => form.value.file_id, async (fileId) => {
  if (!fileId) {
    fileFields.value = []
    referenceFields.value = [...DEFAULT_REF_FIELDS]
    return
  }
  try {
    const res = await getFileFields(fileId)
    fileFields.value = res.fields || []
    referenceFields.value = DEFAULT_REF_FIELDS.filter(f => fileFields.value.includes(f))
  } catch {
    fileFields.value = []
  }
}, { immediate: true })
...
watch(() => form.value.file_id, (newFileId) => {
  ...
  if (file && !form.value.output_filename) {
    form.value.output_filename = buildDefaultOutputFilename(file.filename, 'quality_check', username.value)
  }
})
...
const {
  ...
  saveLoading,
  saveAsNewVersion,
} = usePromptDrawer('quality_check', promptOptions, form, selectedLLMConfigId)
...
const passRateLabel = computed(() => {
  if (!taskInfo.value) return ''
  const pass = taskInfo.value.pass_count || 0
  const fail = taskInfo.value.fail_count || 0
...
async function fetchFileOptions(showAll) {
  try {
    const res = await getQualityCheckSourceFiles({ show_all: showAll })
...
async function fetchPromptOptions() {
  try {
    const res = await getPromptConfigs({ stage: 'quality_check' })
...
async function handleStart() {
  ...
  try {
    ...
    const res = await startQualityCheck(payload)
...
async function handleConfigConfirm(data) {
  ...
  try {
    if (configAction.value === 'retry') {
      if (!taskId.value) return
      const res = await retryStage('quality-check', taskId.value, payload)
      taskId.value = res.task_id
      taskRunning.value = true
      await pollStatus()
      startPolling()
...
async function pollStatus() {
  if (!taskId.value) return
  try {
    const res = await getStageStatus('quality-check', taskId.value)
    taskInfo.value = res
...
async function restoreTaskState() {
  try {
    const tasks = await getTaskList({ stage: 'quality_check' })
