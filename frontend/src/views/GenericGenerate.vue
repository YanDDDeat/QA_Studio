    ...
    <el-card class="form-card">
      ...
      <div class="config-layout">
        <div class="config-form">
          <el-form :model="form" label-width="100px" :disabled="taskRunning">
            <el-form-item label="选择文件">
              <FileSelector v-model="form.file_id" :fetch-fn="fetchFileOptions" :initial-show-all="true" :disabled="taskRunning" />
            </el-form-item>
            ...
            <el-form-item label="选择Prompt">
              <el-select v-model="form.prompt_id" placeholder="请选择任意阶段的Prompt（沙盒模式不限）" style="width: 100%" filterable>
                <el-option v-for="p in promptOptions" :key="p.id" :label="formatPromptLabel(p)" :value="p.id">
                  <span>{{ formatPromptLabel(p) }}{{ p.is_default ? '(默认)' : '' }}</span>
        ...
        <div class="config-preview">
          <PromptPreview :stage="selectedPromptStage" :version="drawerVersion" :prompt-name="drawerPromptName" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event"
    ...
    <el-card v-if="taskInfo" class="progress-card">
      ...
      <div class="progress-area">
        ...
        <div class="progress-text">
          ...
          已生成 {{ taskInfo.processed_count }} 条
        </div>
      </div>
    </el-card>
    ...
    <el-card v-if="taskInfo" class="log-card">
      ...
      <div class="log-area" v-loading="logLoading">
        ...
        >
          ...
          <span :class="log.log_content.includes('处理完成') ? 'log-success' : log.log_content.includes('LLM调用失败') ? 'log-error' : 'log-content'">
            {{ log.log_content }}
          </span>
        </div>
      </div>
...
// 通用生成：Prompt 选择器显示格式 [阶段中文名] Prompt 名称 vN
function formatPromptLabel(p) {
  const label = getStageLabel(p.stage)
  const namePart = p.name || `v${p.version}`
  const versionPart = p.name ? ` v${p.version}` : ''
  return `[${label}] ${namePart}${versionPart}`
}

// 当前选中 Prompt 的 stage（用于 PromptPreview 动态分类）
const selectedPromptStage = computed(() => {
  const p = promptOptions.value.find(p => p.id === form.value.prompt_id)
...
// ----- Prompt drawer -----
// 通用生成沙盒：保存新版本时使用 'generic' stage（与页面定位一致）
const {
  drawerVisible,
  drawerContent,
  drawerVersion,
...
async function fetchFileOptions(showAll) {
  // 通用生成始终请求全部 JSON（show_all 参数忽略，后端默认 True）
  try {
    const res = await getGenericGenerateSourceFiles({ show_all: true })
...
async function fetchPromptOptions() {
  // 通用生成 Prompt 选择器不限 stage：调用 /api/prompts 不传 stage
  try {
...
async function handleConfigConfirm(data) {
  ...
  try {
    if (configAction.value === 'retry') {
      if (!taskId.value) return
      const res = await retryStage('generic-generate', taskId.value, payload)
      taskId.value = res.task_id
      taskRunning.value = true
      await pollStatus()
      startPolling()
...
async function pollStatus() {
  if (!taskId.value) return
  try {
    const res = await getStageStatus('generic-generate', taskId.value)
    taskInfo.value = res
