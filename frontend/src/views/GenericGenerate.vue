<template>
  <div class="page-container">
    ...
    <el-card class="form-card">
      <template #header>
        <span class="card-title">通用生成配置</span>
      </template>
      <div class="config-layout">
        <div class="config-form">
          <el-form :model="form" label-width="100px" :disabled="taskRunning">
            <el-form-item label="选择文件">
              <FileSelector v-model="form.file_id" :fetch-fn="fetchFileOptions" :show-all-default="true" :disabled="taskRunning" />
            </el-form-item>
            ...
            <el-form-item label="选择Prompt">
              <el-select v-model="form.prompt_id" placeholder="请选择Prompt（不限阶段）" style="width: 100%" filterable>
                <el-option v-for="p in promptOptions" :key="p.id" :label="p.displayLabel" :value="p.id">
                  <span>{{ p.displayLabel }}{{ p.is_default ? '(默认)' : '' }}</span>
        ...
        <div class="config-preview">
          <PromptPreview :stage="currentPromptStage" :version="drawerVersion" :prompt-name="drawerPromptName" :content="drawerContent" :time-label="formatTime(drawerCreatedAt)" :content-changed="drawerContentChanged" :next-version="nextVersion" :save-loading="saveLoading" @update:content="drawerContent = $event"
    ...
    <el-card v-if="taskInfo" class="progress-card">
      ...
      <div class="progress-area">
        ...
        <div class="progress-text">
          ...
          已生成 {{ taskInfo.generated_count }} 条
        </div>
      </div>
    </el-card>
    ...
    <el-card v-if="taskInfo" class="log-card">
      ...
      <div class="log-area" v-loading="logLoading">
        ...
        >
          ...
          <span :class="log.log_content.includes('生成完成') ? 'log-success' : log.log_content.includes('失败') ? 'log-error' : 'log-content'">
            {{ log.log_content }}
          </span>
        </div>
      </div>
...
import {
  startGenericGenerate,
  listGenericGenerateSourceFiles,
  retryGenericGenerate,
  getFileFields,
  stopTask,
  resumeTask,
  getStageStatus,
...
import { buildDefaultOutputFilename, getStageLabel } from '../utils/stageLabels'
...
watch(() => form.value.file_id, (newFileId) => {
  ...
  if (file && !form.value.output_filename) {
    form.value.output_filename = buildDefaultOutputFilename(file.filename, 'generic', username.value)
  }
})

// Current prompt's stage (dynamic, used by PromptPreview)
const currentPromptStage = computed(() => {
  const pid = form.value.prompt_id
  const p = promptOptions.value.find(p => p.id === pid)
  return p?.stage || 'generic'
})

// ----- Prompt drawer (stage is dynamic based on selected prompt) -----
const {
  drawerContent,
  drawerVersion,
  drawerCreatedAt,
  ...
  nextVersion,
  saveLoading,
} = usePromptDrawer('generic', promptOptions, form, selectedLLMConfigId)

// Override save to use the selected prompt's actual stage (not hardcoded "generic")
async function saveAsNewVersion() {
  ...
  try {
    const payload = {
      stage: currentPromptStage.value,
    ...
    // Refresh prompt list (fetch all, no stage filter)
    const newPrompts = await getPrompts()
    promptOptions.value = Array.isArray(newPrompts) ? newPrompts.map(p => ({
      ...p,
      displayLabel: `[${getStageLabel(p.stage)}] ${p.name || 'v' + p.version} v${p.version}`,
    })) : []

    form.value.prompt_id = res.id
...
async function fetchFileOptions(showAll) {
  try {
    const res = await listGenericGenerateSourceFiles({ show_all: showAll })
...
async function fetchPromptOptions() {
  try {
    const res = await getPrompts()  // 不传 stage 参数，获取全部 Prompt
    const items = Array.isArray(res) ? res : []
    promptOptions.value = items.map(p => ({
      ...p,
      displayLabel: `[${getStageLabel(p.stage)}] ${p.name || 'v' + p.version} v${p.version}`,
    }))
  } catch (err) {
...
async function handleStart() {
  ...
  try {
    ...
    const res = await startGenericGenerate(payload)
...
async function handleConfigConfirm(data) {
  ...
  try {
    if (configAction.value === 'retry') {
      if (!taskId.value) return
      const res = await retryGenericGenerate(taskId.value, payload)
      taskId.value = res.task_id
      taskRunning.value = true
      await pollStatus()
      startPolling()
...
async function pollStatus() {
  if (!taskId.value) return
  try {
    const res = await getStageStatus('generic_generate', taskId.value)
    taskInfo.value = res
...
async function restoreTaskState() {
  try {
    const tasks = await getTaskList({ stage: 'generic' })
