"""Answer Validate router - Pipeline Stage 5

Validates all Dataset records in a JSON file from the answer_generate
...
class AnswerValidateStartRequest(BaseModel):
...
async def _run_answer_validate_task(
    task_id: int,
    file_id: int,
    prompt_content: str,
    model: str,
    ...
    try:
        ...
        while idx < total:
            ...
            record_content = build_record_content(dataset, reference_fields, "answer_validate")
...
@router.post("/start")
async def start_answer_validate(
    data: AnswerValidateStartRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Start an answer validation task.
    ...
    is_valid, validation_msg, validation_stats = validate_file_fields(
        file_obj.file_path, "answer_validate"
    )
    if not is_valid:
        raise HTTPException(
    ...
    if prompt_obj.stage != StageEnum.ANSWER_VALIDATE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="提示词必须为answer_validate阶段",
        )
...
async def get_answer_validate_status(
    task_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Get the status and progress of an answer validation task.
...
async def retry_answer_validate(
    task_id: int,
...
def resume_answer_validate_task(task: Task, db: Session, llm_config_id=None):
