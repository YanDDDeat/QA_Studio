"""Generic Generate router - Open sandbox stage

通用生成阶段：不限制上游 source_stage 和 Prompt stage，无固定必填字段，
LLM 返回什么字段就自动映射什么字段。适合自由探索和实验性任务。

Key design (no-overwrite pattern):
- User selects a JSON file (file_id) and a Prompt (any stage)
- Background task processes all Dataset records linked to that file
- Source Dataset records and disk files are NEVER modified
- Generated records: cloned to a new output File with source_stage=GENERIC
...
class GenericStartRequest(BaseModel):
    file_id: int = Field(..., description="ID of the JSON file to process")
    prompt_id: int = Field(..., description="ID of the prompt to use (any stage)")
...
async def _run_generic_task(
    ...
    """Background coroutine: 对每条 Dataset 调用 LLM，克隆到新输出文件并自动映射字段。
    ...
    try:
        ...
        if generated_count == 0:
            _add_task_log(db, task_id, "本次任务无成功生成记录")
        else:
            write_datasets_to_file(db=db, file_id=output_file.id)
            _add_task_log(db, task_id, f"生成文件已就绪: {output_file.filename}")
        ...
        logger.info(
            "Task %d completed: processed %d records, generated %d",
...
async def start_generic_generate(
    data: GenericStartRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Start a generic generation task.

    Validates the file and prompt (any stage), creates a Task record, and
    launches a background coroutine that processes each Dataset record
    ...
    # NOTE: 通用生成不限 prompt stage（与 data_evaluate 的核心区别）
...
async def get_generic_generate_status(
    ...
    """Get the status and progress of a generic generation task."""
    task = (
        db.query(Task)
...
async def list_source_files(
    ...
    默认返回所有 JSON 文件（不限上游 stage），show_all 默认为 True。
    """
    ...
    if not show_all:
        query = query.filter(File.source_stage == StageEnum.GENERIC)
...
async def retry_generic_generate(
    ...
    """Retry a failed or completed generic generation task."""
    task = (
        db.query(Task)
        .filter(Task.id == task_id, Task.user_id == current_user.id)
        .first()
...
def resume_generic_task(task: Task, db: Session, llm_config_id=None):
    """Resume a paused generic task from progress_current."""
    source_fid = task.source_file_id or task.file_id
    file_obj = (
        db.query(File)
