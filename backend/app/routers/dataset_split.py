...
async def _run_dataset_split_task(
    ...
    try:
        ...
        _add_task_log(db, task_id, f"切分完成: 测试集 {result['test_count']} 条, 训练集 {result['train_count']} 条, 非QA自动归入训练集 {result['skipped_non_qa']} 条 | 测试集题型: {result['test_task_counts']} | 训练集题型: {result['train_task_counts']}")
...
async def list_source_files(
    ...
    """List JSON files for dataset splitting.

    默认只返回 source_stage=cot_filter 的文件；show_all=True 时返回所有 JSON。
    """
    ...
    if not show_all:
        query = query.filter(File.source_stage == StageEnum.COT_FILTER)
