...
Validates all Dataset records in a JSON file from the data_evaluate
...
async def _run_quality_check_task(
    ...
    try:
        ...
        if start_index == 0:
            ...
            if task_obj and task_obj.source_file_id:
                source_file = db.query(File).filter(File.id == task_obj.source_file_id).first()
